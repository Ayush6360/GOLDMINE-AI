import { dbGet, dbAll, dbRun, dbBatch } from "@/lib/data/db";
...
/**
 * Ingestion service: pulls from external sources and persists to libSQL. This is
...
export async function runIngestion(): Promise<IngestReport> {
  ...
  try {
    ...
    await dbBatch(
      bars.map((b) => ({
        sql: `INSERT INTO prices (asset, date, close, source, fetched_at)
              VALUES (?, ?, ?, 'yahoo:GC=F', ?)
              ON CONFLICT(asset, date) DO UPDATE SET close = excluded.close, fetched_at = excluded.fetched_at`,
        args: ["gold", b.date, b.close, startedAt],
      })),
    );
    report.pricesUpserted = bars.length;
  } catch (e) {
  ...
  for (const [indicator, symbol] of yahooMacro) {
    try {
      ...
      await upsertMacro(indicator, last.date, adjustMacro(indicator, last.close), `yahoo:${symbol}`, startedAt);
  ...
  if (hasFred()) {
    try {
      ...
      if (cpi !== null) {
        await upsertMacro("cpi_yoy", startedAt.slice(0, 10), cpi, "fred:CPIAUCSL", startedAt);
        report.macroUpserted++;
      }
      const real = await fetchLatest(FRED_SERIES.realYield10y);
      if (real) {
        await upsertMacro("real_yield_10y", real.date, real.value, "fred:DFII10", startedAt);
        report.macroUpserted++;
      }
      const rate = await fetchLatest(FRED_SERIES.policyRate);
      if (rate) {
        await upsertMacro("policy_rate", rate.date, rate.value, "fred:FEDFUNDS", startedAt);
        report.macroUpserted++;
      }
    } catch (e) {
  ...
  try {
    const gold = await dbGet<{ close: number }>(
      `SELECT close FROM prices WHERE asset='gold' ORDER BY date DESC LIMIT 1`,
    );
    const fx = await dbGet<{ value: number }>(
      `SELECT value FROM macro WHERE indicator='usdinr' ORDER BY date DESC LIMIT 1`,
    );
    if (gold) {
      // Direction lean from a quick momentum read (cheap; avoids a full engine call).
      const recent = await dbAll<{ close: number }>(
        `SELECT close FROM prices WHERE asset='gold' ORDER BY date DESC LIMIT 6`,
      );
      const probUp = recent.length >= 2 && recent[0].close >= recent[recent.length - 1].close ? 0.55 : 0.45;
      const fired = await evaluateAlerts(gold.close, fx?.value ?? 96, probUp);
  ...
  await dbRun(
    "INSERT INTO ingest_runs (started_at, status, detail) VALUES (?, ?, ?)",
    [startedAt, report.status, JSON.stringify({ ...report, errors: report.errors.slice(0, 10) })],
  );
...
async function upsertMacro(indicator: string, date: string, value: number, source: string, fetchedAt: string): Promise<void> {
  await dbRun(
    ...
    [indicator, date, value, source, fetchedAt],
  );
}
